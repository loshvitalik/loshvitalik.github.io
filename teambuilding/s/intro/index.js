...
const DEFAULT_BUTTONS = [YES, NO, NO_ANSWER];
...
setTimeout(() => addChatMessage("Ты же Кристина, да?", "K", DEFAULT_BUTTONS), NEXT_MESSAGE_DELAY * 2);
  ...
  if (tag === "K") {
    switch (button) {
      case YES:
        ...
        break;
      case NO:
        addChatMessage("Значит, ты &ndash; Валерия?", "V", DEFAULT_BUTTONS);
        break;
      case NO_ANSWER:
        addChatMessage("Ты же Кристина, да?", "K", DEFAULT_BUTTONS);
        break;
    }
  } else if (tag === "V") {
    switch (button) {
...
function navigateToInvite(tag) {
  ...
  var location = tag === "K" ? "../invite" : "../invite"; // "../../invite"
